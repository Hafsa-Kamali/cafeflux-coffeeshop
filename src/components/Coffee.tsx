...
const specificSlugs = ["9", "11","8"]; // Specific products to fetch
    ...
    <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {products.map((product, index) => (
        <Link key={product.slug} href={`/products/${product.slug}`} passHref>
          <div
            className={`group relative rounded-xl overflow-hidden shadow-xl shadow-[#38220f] transition-all duration-500 ease-linear transform hover:-translate-y-1 cursor-pointer ${index % 2 === 0 ? 'bg-[#967259]' : 'bg-[#634832]'}`}
          >
